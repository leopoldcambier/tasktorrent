...
TEST(Ttor, MiniV0)
{
    ...
    Threadpool tp(n_threads, comm.get(), VERB, "WkMini_v0_" + to_string(rank) + "_");
    ...
    comms_world_barrier(); // This is required in case we call this function repeatedly
}
...
TEST(Ttor, MiniV1)
{
    ...
    Threadpool tp(n_threads, comm.get(), VERB, "WkMini_v1_" + to_string(rank) + "_");
    Taskflow<int> tf(&tp, VERB);
...
void test_ttor_mini(int n_tasks_per_rank, bool self)
{
    ...
    Threadpool tp(n_threads, comm.get(), VERB, "WkMini_" + to_string(rank) + "_");
    ...
    if (VERB > 2)
    {
        ...
        logfile.open("commdag.log." + to_string(comms_world_rank()));
        logfile << log;
        logfile.close();
    }
...
TEST(Ttor, Bound)
{
    ...
    Threadpool tp(n_threads, comm.get(), VERB, "WkCritical_1_" + to_string(rank) + "_");
...
TEST(Ttor, Bound2)
{
    ...
    Threadpool tp(n_threads, comm.get(), VERB, "WkCritical_2_" + to_string(rank) + "_");
    Taskflow<int> tf_0(&tp, VERB);
    Taskflow<int> tf_1(&tp, VERB);
...
void test_sparse_graph(int n_tasks_per_rank, bool self)
{
    ...
    Threadpool tp(n_threads, comm.get(), VERB, "WkSparseDAG_" + to_string(rank) + "_");
    Taskflow<int> tf(&tp, VERB);
...
TEST(Ttor, Ring)
{
    ...
    Threadpool tp(2, comm.get(), VERB, "WkRing_" + to_string(rank) + "_");
    ...
    auto am = comm->make_active_msg(
        [&](const int &round, const int &payload) {
...
TEST(Ttor, PingPong)
{
    ...
    int rank = comms_world_rank();
    if(n_ranks % 2 == 1) {
        cout << "Pingpong skipped because odd number of ranks" << endl;
    } else {
        ...
        Threadpool tp(n_threads, comm.get(), VERB, "Wk_" + to_string(rank) + "_");
        ...
        auto am = comm->make_active_msg(
            [&](const int &round, const int &new_ball) {
                ball = new_ball;
